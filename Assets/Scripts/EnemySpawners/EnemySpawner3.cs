...
public class EnemySpawner3 : MonoBehaviour {
    ...
    public EnemySpawner2 SeceondEnemySpawner;
    void Start()
    {
        SeceondEnemySpawner = GameObject.FindObjectOfType<EnemySpawner2>();
        StartupConstraints();        
    ...
    void Update () {
        ...
        RoundControlls();
        if (SeceondEnemySpawner.ThirdFormationSpawnStart == true)
        {
    ...
    {        
        if ((SeceondEnemySpawner.ThirdFormationSpawnStart == true) && (EnemySpawner.RoundsDone < 2))
        {
            print("WWW---------------------------------------------------------");
            foreach (Transform child in transform)
            {
                ...
                EnemySpawner.EnemyCounter += 1;
                this.transform.position = new Vector3(transform.position.x, MoveDownStart,0);
                SeceondEnemySpawner.ThirdFormationSpawnStart = false;
            }
