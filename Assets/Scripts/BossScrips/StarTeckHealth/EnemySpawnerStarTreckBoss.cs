...
public class EnemySpawnerStarTreckBoss : MonoBehaviour {
    public GameObject StarTreckShipShip;
    ...
    void Start()
    {
        ...
        {            
            GameObject StarTreckEnterprise = Instantiate(StarTreckShipShip, child.transform.position, Rottater.transform.rotation) as GameObject;
            StarTreckEnterprise.transform.parent = child;
            EnemyCounter += 1;
        }
    }
    ...
    {        
        if ((EnemyCounter == 0) && (RoundsDone < 3))
        {
            print("qqq---------------------------------------------------------");
            foreach (Transform child in transform)
            {
